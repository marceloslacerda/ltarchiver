...
class MyTestCase(unittest.TestCase):
    def test_file_not_exists_no_recordbook(self):
        ...
        self.assertRaises(
            FileNotFoundError, store.file_not_exists, "", pathlib.Path("nopath")
        )

    def test_file_not_exists_file_in_recordbook(self):
        write_test_recorbook(common.recordbook_path)
        self.assertRaises(
            common.LTAError, store.file_not_exists, "bogus md5", TEST_SOURCE_FILE.name
        )
        self.assertRaises(
            common.LTAError, store.file_not_exists, TEST_FILE_CHECKSUM, "bogus name"
        )
    ...
    def test_sync_recordbooks_no_file(self):
        ...
        os.mkdir("test_data")
        self.assertRaises(
            FileNotFoundError, store.sync_recordbooks, pathlib.Path("test_data")
        )
